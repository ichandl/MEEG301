...
w_nl = 15100         # no-load speed [rpm]
i_nl = 0.662         # no-load current [A]
T_nom = 44.6         # nominal torque [mNm]
w_nom = 13400        # nominal speed [rpm]
T_stall = 428        # stall torque [mNm]
kt = 7.48            # torque constant [mNm/A]
w_max = 25000        # max speed [rpm]

# loading condition (to be driven through gearhead)
T_load = 1600        # load torque [mNm]
w_load = 40          # load speed [rpm]
eta_gear = 0.70      # gearhead efficiency
...
for i in range(w.size):
    V[:,i] = V_nom*(T/T_stall + w[i]/w_nl)                   # calculate voltage at this T and w [V]
...
plt.title('Maxon 118891 motor', fontsize=28)
...
gear_ratio = np.linspace(1,500,201)
gear_ratio_markers = np.geomspace(1,500,11)
